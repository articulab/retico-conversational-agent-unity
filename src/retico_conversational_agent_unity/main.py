...
from retico_conversational_agent_unity.ASR_DM import AsrDmModule
from retico_conversational_agent_unity.LLM_DM import LlmDmModule
from retico_conversational_agent_unity.TTS_DM import TtsDmModule
from retico_conversational_agent_unity.Speaker_DM import SpeakerDmModule
from retico_conversational_agent_unity.dialogue_manager import (
...
def main_DM_unity_2():
    ...
    # gesture_demo = GestureDemoModule()
    nvg = NonverbalGeneratorModule(
        tts_framerate=tts_model_samplerate,
    )

    unity_comm = UnityCommunicatorModule()
    ...
    unity_comm.subscribe(bridge)
    bridge.subscribe(aw)
    aw.subscribe(ar)  # just so that they are on the same network
    ar.subscribe(unity_comm)
    unity_comm.subscribe(llm)  # to align dialogue history
